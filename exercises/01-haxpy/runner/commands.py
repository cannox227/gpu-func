...
class Options:
    ...
    sm_target: Optional[str] = None    # CUDA SM target, e.g. "90"; None -> nvcc default
...
class Command:
    ...
    def _with_target(self, cmd: CompilerCommand) -> CompilerCommand:
        """Apply the configured SM target, if any, to a compiler command."""
        if self.options.sm_target is not None:
            cmd = cmd.with_target(self.options.sm_target)
        return cmd
...
class CompileCommandBase(Command):
    ...
    def _exec(self, tests: List[str]) -> bool:
        out = self._out()
        cmd = self._with_target(self._build_command(out))
...
class RunCommandBase(Command):
    ...
    def _compile(self) -> Optional[Path]:
        ...
        command = self._with_target(command)
...
class ProfileCommand(RunCommandBase):
    ...
    def report_result(self, name, config: TestConfig, report: Report) -> None:
        ...
        if config.name in self.exercise.key_benchmarks:
